...
// Elimina o modifica esta función para que no convierta a céntimos
const needsConversion = (value: string): boolean => {
    // Siempre retorna false para tratar todos los valores como euros
    return false;
    // La antigua implementación era:
...
function handleBlur(el: HTMLInputElement) {
    ...
    // Elimina la conversión a céntimos
    // if (!value.includes(".")) {
...
export const currencyDirective = {
    ...
    updated(el: HTMLInputElement) {
        if (!el.matches(":focus")) {
            ...
            el.dataset.rawValue = rawValue;
            const value = rawValue || "0";
            // Elimina la conversión a céntimos aquí también
